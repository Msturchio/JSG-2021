...
par(new=T)
plotCI(sdf$Location, sdf$SWC.mean, sdf$SWC.std.error, sfrac = 0,
       xaxt="n",yaxt="n",xlab="",ylab="",pch=16, cex = 1.7, col = "dodgerblue4", ylim = c(18,34))

points(sdf$SWC.mean ~ sdf$Location, col = "dodgerblue4", lty = 1, lwd = 2, type = "l")

axis(2, at = seq(18,34,2), las = 2, cex.axis = 1.4)
mtext(side = 2, expression("SWC%"), cex = 1.3, padj = 1, las = 2, outer= T)
legend("topright", c("SWC%"), col=c("dodgerblue4"), pch= c(19), cex = 1.5, horiz = F, bty='n')
